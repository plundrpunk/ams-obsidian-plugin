...
## 2026-05-10 - Length-based fast-paths for string validation
**Learning:** Calling `.trim()` on large string contents like entire vault files allocates large temporary strings, causing unnecessary GC overhead when validating those files against short literal strings.
**Action:** When validating potentially large strings against short literals, use a length-based fast-path (`if (content.length > 100) return true;`) to avoid string copying methods.
